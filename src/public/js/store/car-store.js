...
async function delete_all_car(total, moneyReceived, exchange, comment) {
    ...
    try {
        ...
        const link = '/fud/car-customer-post';
        const answerServer = await get_answer_server_store(combos, link);
...
async function get_answer_server_for_add_order(idCommander) {
    try {
        ...
        if (!response.ok) {
            throw new Error('Error en la solicitud de la orden');
        }
...
async function buy_my_car_store(button) {
    var dataOrder = document.getElementById('customer-name').value;
    if (dataOrder === ""){
